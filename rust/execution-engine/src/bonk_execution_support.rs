...
use shared_transaction_submit::{
    CompiledTransaction, compiled_transaction_signers, fetch_account_data,
    fetch_account_data_with_owner, fetch_latest_blockhash_cached, fetch_multiple_account_data,
...
fn configured_default_follow_up_compute_unit_limit() -> u64 {
    configured_compute_unit_limit_env(
...
fn configured_default_usd1_sell_to_sol_compute_unit_limit() -> u64 {
    configured_compute_unit_limit_env(
        "LAUNCHDECK_BONK_USD1_SELL_TO_SOL_COMPUTE_UNIT_LIMIT",
        DEFAULT_BONK_SELL_COMPUTE_UNIT_LIMIT,
    )
    .max(configured_default_launchpad_sell_compute_unit_limit())
}

fn configured_default_launch_usd1_topup_compute_unit_limit() -> u64 {
    configured_compute_unit_limit_env(
...
fn execution_transport_config(
    execution: &NormalizedExecution,
) -> crate::transport::ExecutionTransportConfig {
    crate::transport::ExecutionTransportConfig {
        provider: execution.provider.clone(),
        endpoint_profile: execution.endpointProfile.clone(),
        commitment: execution.commitment.clone(),
        skip_preflight: execution.skipPreflight,
        track_send_block_height: execution.trackSendBlockHeight,
        mev_mode: execution.mevMode.clone(),
        mev_protect: !execution.mevMode.eq_ignore_ascii_case("off"),
    }
}
...
#[derive(Debug, Clone)]
pub struct TrustedRaydiumClmmSwap {
...
async fn rpc_fetch_bonk_raydium_pools_for_ordered_pair(
    ...
    let payload = json!({
        "jsonrpc": "2.0",
        "id": "execution-engine-bonk-raydium-pools",
...
fn load_persisted_bonk_lookup_table_account(address: &str) -> Option<AddressLookupTableAccount> {
    ...
    if entry.address_count != Some(entry.addresses.len()) {
        eprintln!(
            "[execution-engine][alt-cache] ignoring stale Bonk ALT snapshot {} due to missing/mismatched address count",
            address
        );
        return None;
    }
    ...
    if entry.content_hash.as_deref() != Some(content_hash.as_str()) {
        eprintln!(
            "[execution-engine][alt-cache] ignoring stale Bonk ALT snapshot {} due to content hash mismatch",
            address
        );
        return None;
    }
...
pub async fn build_trusted_raydium_clmm_swap_exact_in(
    ...
    let expected_out = biguint_to_u64(&quote.expected_out, "trusted Raydium CLMM expected output")?;
    let min_out = biguint_to_u64(&quote.min_out, "trusted Raydium CLMM minimum output")?;
    if expected_out == 0 || min_out == 0 {
        return Err("Trusted stable Raydium quote returned zero output.".to_string());
    }
...
#[derive(Debug, Clone)]
struct CachedBonkHoldingQuoteContext {
    fetched_at: Instant,
    context: NativeBonkTradeVenueContext,
}

fn bonk_holding_quote_context_cache()
-> &'static tokio::sync::Mutex<HashMap<String, CachedBonkHoldingQuoteContext>> {
    static CACHE: OnceLock<tokio::sync::Mutex<HashMap<String, CachedBonkHoldingQuoteContext>>> =
        OnceLock::new();
    CACHE.get_or_init(|| tokio::sync::Mutex::new(HashMap::new()))
}

fn bonk_holding_quote_context_key(
    rpc_url: &str,
    mint: &str,
    pool_id: Option<&str>,
    quote_asset: &str,
    ...
    format!(
        "rpc={}|cmt={}|mint={}|pool={}|quote={}",
        rpc_url,
        commitment,
        mint,
        pool_id.unwrap_or("").trim(),
        quote_asset
    )
}

fn quote_bonk_context(
    context: &NativeBonkTradeVenueContext,
    mint_pubkey: &Pubkey,
    ...
    match context {
        NativeBonkTradeVenueContext::Launchpad(context) => {
            let (expected_out, _) = bonk_follow_sell_quote_amounts(context, token_amount_raw, 0)?;
            Ok((expected_out, context.quote.asset.to_string()))
        }
        NativeBonkTradeVenueContext::RaydiumCpmm(context) => {
            let (expected_out, _) =
                bonk_cpmm_quote_exact_input(context, mint_pubkey, token_amount_raw, 0)?;
            Ok((expected_out, context.quote.asset.to_string()))
        }
        NativeBonkTradeVenueContext::RaydiumClmm(context) => {
            let quote = bonk_quote_clmm_exact_input(
                &context.setup,
                mint_pubkey,
                &bonk_biguint_from_u64(token_amount_raw),
                0,
            )?;
            Ok((
                biguint_to_u64(&quote.expected_out, "Bonk CLMM holding quote output")?,
                context.quote.asset.to_string(),
            ))
        }
    }
}

pub async fn quote_bonk_holding_value_quote_raw(
    rpc_url: &str,
    mint: &str,
    pool_id: Option<&str>,
    quote_asset: &str,
    ...
    let cache_key = bonk_holding_quote_context_key(rpc_url, mint, pool_id, quote.asset, commitment);
    {
        ...
        {
            return quote_bonk_context(&entry.context, &mint_pubkey, token_amount_raw);
        }
    }
    let venue_context = if let Some(pool_id) =
        pool_id.map(str::trim).filter(|value| !value.is_empty())
    {
        load_bonk_trade_venue_context_by_pool_id(rpc_url, pool_id, quote.asset, commitment).await?
    } else {
    ...
    };
    {
        ...
        cache.insert(
            cache_key,
            CachedBonkHoldingQuoteContext {
                fetched_at: Instant::now(),
                context: venue_context.clone(),
            },
        );
        if cache.len() > 256 {
    ...
    quote_bonk_context(&venue_context, &mint_pubkey, token_amount_raw)
}
...
async fn native_compile_bonk_buy_transaction_with_pool_context(
    ...
    let tx_config = bonk_follow_tx_config(
        configured_default_launchpad_buy_compute_unit_limit(),
...
async fn native_compile_bonk_sell_transaction_with_cpmm_context(
    ...
    let compute_unit_limit = if pool_context.quote.asset == "usd1" && settlement_asset == "sol" {
        configured_default_usd1_sell_to_sol_compute_unit_limit()
    } else {
        configured_default_raydium_sell_compute_unit_limit()
    };
    let tx_config = bonk_follow_tx_config(
        compute_unit_limit,
    ...
    if quote.asset == "usd1" {
        ...
        if funding_policy == "sol_only" || current_balance < requested_amount_b {
            ...
            .await
            {
                ...
                Err(error) => {
                    let transport_plan = crate::transport::build_transport_plan(
                        &execution_transport_config(execution),
                        2,
                    );
                    if transport_plan.execution_class != "bundle"
                        && transport_plan.ordering != "bundle"
                    {
                        return Err(format!(
                            ...
                            transport_plan.transport_type, error
                        ));
                    }
...
fn bonk_usd1_short_quote_setup_cache()
-> &'static Mutex<HashMap<String, BonkUsd1RouteSetupCacheEntry>> {
    static CACHE: OnceLock<Mutex<HashMap<String, BonkUsd1RouteSetupCacheEntry>>> = OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}

pub async fn quote_sol_lamports_for_exact_usd1_input_with_max_setup_age(
    rpc_url: &str,
    usd1_raw: u64,
    max_setup_age: Duration,
) -> Result<u64, String> {
    if usd1_raw == 0 {
        return Ok(0);
    }
    let cache_key = rpc_url.to_string();
    let cached = bonk_usd1_short_quote_setup_cache()
        .lock()
        .expect("bonk usd1 short quote setup cache")
        .get(&cache_key)
        .filter(|entry| entry.fetched_at.elapsed() <= max_setup_age)
        .cloned();
    let setup = if let Some(entry) = cached {
        entry.setup
    } else {
        let setup = load_bonk_usd1_route_setup_fresh(rpc_url).await?;
        {
            let mut cache = bonk_usd1_short_quote_setup_cache()
                .lock()
                .expect("bonk usd1 short quote setup cache");
            cache.insert(
                cache_key,
