...
use shared_execution_routing::provider_tip::HELIUS_SENDER_TIP_ACCOUNTS;
...
async fn cached_hellomoon_quic_client(
    ...
    let client = Arc::new(
        timeout(
            HELLOMOON_QUIC_CONNECT_TIMEOUT,
        ...
        .map_err(|_| {
            format!(
                "Hello Moon QUIC connect timed out after {}s for {endpoint}",
                HELLOMOON_QUIC_CONNECT_TIMEOUT.as_secs()
            )
        })?
...
async fn refresh_block_height_sample(rpc_url: &str, commitment: &str) -> Result<u64, String> {
    ...
    .await
    {
        ...
        Err(error) if block_height_rpc_url != rpc_url => {
            ...
            eprintln!(
                "[shared-transaction-submit][rpc] warm RPC getBlockHeight unavailable; using primary sample"
            );
            (
                rpc_request(
                    rpc_url,
...
async fn refresh_slot_sample(rpc_url: &str, commitment: &str) -> Result<u64, String> {
    ...
    .await
    {
        ...
        Err(error) if slot_rpc_url != rpc_url => {
            ...
            eprintln!(
                "[shared-transaction-submit][rpc] warm RPC getSlot unavailable; using primary sample"
            );
            (
...
const WEBSOCKET_CONFIRMATION_TIMEOUT_SECS: u64 = 10;
const BATCH_CONFIRMATION_POLL_MAX_ATTEMPTS: u32 = 10;
const BATCH_CONFIRMATION_POLL_INTERVAL_MS: u64 = 500;
...
impl ConfirmWatchError {
    fn from_confirmation_error(error: String) -> Self {
        if is_terminal_confirmation_error(&error) {
            Self::Terminal(error)
        } else {
            Self::WatchUnavailable(error)
        }
    }
        ...
        match self {
            Self::NotObserved {
                ...
                if *signature_count == 1 {
                    format!("Submitted transaction was not observed by {source} before timeout.")
                } else {
                    format!(
                        "Submitted transactions were not observed by {source} before timeout ({signature_count} transaction(s))."
                    )
                }
            }
            Self::WatchUnavailable(message) | Self::Terminal(message) => message.clone(),
        }
    }
}

fn should_fallback_after_watch_error(error: &ConfirmWatchError) -> bool {
    matches!(
        error,
        ConfirmWatchError::WatchUnavailable(_) | ConfirmWatchError::NotObserved { .. }
    )
}
    ...
    if let Some(endpoint) = preferred_confirmation_websocket_endpoint(None) {
        ...
        if prefer_helius_transaction_subscribe && !account_required.is_empty() {
            ...
            .await
            {
                ...
                Err(ConfirmWatchError::Terminal(error)) => return Err(error),
                Err(error @ ConfirmWatchError::NotObserved { .. }) => return Err(error.message()),
                Err(ConfirmWatchError::WatchUnavailable(error)) => {
                    if let Some(standard_endpoint) = standard_endpoint.as_deref() {
                        ...
                        .await
                        {
                            Ok(confirmation) => return Ok(confirmation),
                            Err(ConfirmWatchError::Terminal(error)) => return Err(error),
                            Err(error @ ConfirmWatchError::NotObserved { .. }) => {
                                return Err(error.message());
                            }
                            Err(ConfirmWatchError::WatchUnavailable(standard_error)) => {
                                eprintln!(
                                    "Standard websocket unavailable after Helius transactionSubscribe failure: {standard_error}; original Helius error: {error}"
                                );
                            }
                        }
                    }
            ...
            if let Some(standard_endpoint) = standard_endpoint.as_deref() {
                ...
                .await
                {
                    ...
                    Err(ConfirmWatchError::WatchUnavailable(_)) => {}
                }
            }
        } else {
            match wait_for_confirmation_websocket(
...
async fn wait_for_confirmation_websocket(
    ...
    let session = async {
        let mut ws = open_subscription_socket(endpoint)
            .await
            .map_err(ConfirmWatchError::WatchUnavailable)?;
        subscribe(
            &mut ws,
            "signatureSubscribe",
            json!([
        ...
        .map_err(ConfirmWatchError::WatchUnavailable)?;
        loop {
            let message = next_json_message(&mut ws)
                .await
                .map_err(ConfirmWatchError::WatchUnavailable)?;
            if let Some((_, context_slot, value)) = extract_signature_notification(&message) {
                ...
                .await
                .map_err(ConfirmWatchError::from_confirmation_error);
            }
        }
    };
    timeout(
    ...
    .await
    .map_err(|_| {
        ConfirmWatchError::WatchUnavailable(format!(
            "Timed out waiting for standard websocket confirmation for transaction {signature}."
        ))
    })?
}
...
async fn wait_for_confirmation_helius_transaction_subscribe(
    ...
    let session = async {
        ...
        loop {
            ...
            {
                Ok(message) => {
                    let message = message.map_err(ConfirmWatchError::WatchUnavailable)?;
    ...
    .map_err(|_| {
        ConfirmWatchError::WatchUnavailable(format!(
            "Timed out waiting for Helius transactionSubscribe confirmation for transaction {signature}."
        ))
    })?
}
...
async fn wait_for_confirmations_websocket_batch(
    ...
    commitment: &str,
    track_confirmed_block_height: bool,
) -> Result<(Vec<(usize, ConfirmationDetails)>, Vec<String>), ConfirmWatchError> {
    let session = async {
        let mut ws = open_subscription_socket(endpoint)
            .await
            .map_err(ConfirmWatchError::WatchUnavailable)?;
        let (subscription_map, mut buffered_notifications) =
            subscribe_signature_batch(&mut ws, signatures, commitment)
                .await
                .map_err(ConfirmWatchError::WatchUnavailable)?;
        let mut warnings = Vec::new();
            ...
            if let Some((amount, observed_slot)) = watched_account_balances.get(&index).cloned() {
                ...
                confirmation.confirmed_token_balance_raw =
                    if let Some(account) = account_watch_by_index.get(&index).copied() {
                        reconcile_token_account_balance_raw(rpc_url, account)
                            .await
                            .map_err(ConfirmWatchError::from_confirmation_error)?
                    } else {
                        None
    ...
    .map_err(|_| {
        ConfirmWatchError::WatchUnavailable(format!(
            "Timed out waiting for standard websocket confirmation for {} transaction(s).",
            signatures.len()
        ))
    })?
}
...
async fn wait_for_confirmations_helius_transaction_subscribe_batch(
    ...
    commitment: &str,
    track_confirmed_block_height: bool,
) -> Result<Vec<(usize, ConfirmationDetails)>, ConfirmWatchError> {
    let session = async {
        let mut ws = open_subscription_socket(endpoint)
            .await
            .map_err(ConfirmWatchError::WatchUnavailable)?;
        let (subscription_map, mut buffered_notifications) =
            subscribe_helius_transaction_batch(&mut ws, requests, commitment)
                .await
                .map_err(ConfirmWatchError::WatchUnavailable)?;
        ...
        while !pending.is_empty() {
            let message = if let Some(message) = buffered_notifications.pop_front() {
                ...
                .await
                {
                    Ok(message) => message.map_err(ConfirmWatchError::WatchUnavailable)?,
                    Err(_) => {
            ...
            let request_signature = requests
                ...
                .ok_or_else(|| {
                    ConfirmWatchError::WatchUnavailable(format!(
                        "Missing Helius request for index {index}"
                    ))
                })?;
            confirmations.push((
                index,
        ...
        if prefer_helius_transaction_subscribe {
            ...
            if helius_requests_ready {
                ...
                .await
                {
                    ...
                    Err(ConfirmWatchError::WatchUnavailable(helius_error)) => {
                        warnings.push(format!(
                            "Helius transactionSubscribe batch watch unavailable via {} for {} transaction(s): {}. Falling back to standard websocket.",
                            endpoint,
                            signatures.len(),
                            helius_error
                        ));
                        ...
                        .await
                        {
                            ...
                            Err(websocket_error) => match websocket_error {
                                ConfirmWatchError::Terminal(error) => return Err(error),
                                error @ ConfirmWatchError::NotObserved { .. } => {
                                    return Err(error.message());
                                }
                                ConfirmWatchError::WatchUnavailable(websocket_error) => {
                                    warnings.push(format!(
                                    "Standard websocket batch watch unavailable via {} for {} transaction(s): {}. Falling back to batched RPC polling.",
                                    standard_endpoint,
                                    signatures.len(),
                                    websocket_error
                                ));
            ...
            .await
            {
                ...
                Err(error) => match error {
                    ConfirmWatchError::Terminal(error) => return Err(error),
                    error @ ConfirmWatchError::NotObserved { .. } => return Err(error.message()),
                    ConfirmWatchError::WatchUnavailable(error) => {
                        warnings.push(format!(
                        "Websocket batch watch unavailable via {} for {} transaction(s): {}. Falling back to batched RPC polling.",
                        endpoint,
                        signatures.len(),
                        error
                    ));
...
async fn submit_single_transaction_hellomoon_quic(
    ...
    while let Some((endpoint, result)) = endpoint_results.next().await {
        match result {
            Ok(()) => {
                eprintln!(
                    "[shared-transaction-submit][hellomoon-quic] endpoint accepted label={} endpoint={} signature={} compute_unit_price_micro_lamports={:?} inline_tip_lamports={:?} inline_tip_account={:?}",
                    transaction.label,
                    endpoint,
                    signature,
                    transaction.computeUnitPriceMicroLamports,
                    transaction.inlineTipLamports,
            ...
            Err(error) => {
                eprintln!(
                    "[shared-transaction-submit][hellomoon-quic] endpoint failed label={} endpoint={} error={}",
                    transaction.label, endpoint, error
                );
...
mod tests {
    ...
    // removal under the module's env mutex so we aren't racing other tests in
    // this file that manipulate process env.
    ...
    #[test]
    fn watch_timeout_triggers_fallback_policy() {
        let error = ConfirmWatchError::NotObserved {
            source: "standard websocket",
            signature_count: 1,
        };
        assert!(should_fallback_after_watch_error(&error));
        assert!(error.message().contains("not observed"));
    }

    #[test]
    fn watch_unavailable_triggers_fallback_policy() {
        let error = ConfirmWatchError::WatchUnavailable("subscribe failed".to_string());
        assert!(should_fallback_after_watch_error(&error));
        assert_eq!(error.message(), "subscribe failed");
    }

    #[test]
    fn terminal_watch_error_does_not_trigger_fallback_policy() {
        let error =
            ConfirmWatchError::Terminal("Transaction abc failed on-chain: null".to_string());
        assert!(!should_fallback_after_watch_error(&error));
        assert_eq!(error.message(), "Transaction abc failed on-chain: null");
    }

    #[test]
    fn warm_rpc_url_uses_global_env_and_ignores_legacy_alias() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        unsafe {
            std::env::set_var("WARM_RPC_URL", "https://warm.example");
            std::env::set_var("LAUNCHDECK_WARM_RPC_URL", "https://legacy.example");
        }
        assert_eq!(
            configured_warm_rpc_url("https://primary.example"),
            "https://warm.example"
        );
        ...
        unsafe {
            std::env::remove_var("LAUNCHDECK_WARM_RPC_URL");
        }
    }
