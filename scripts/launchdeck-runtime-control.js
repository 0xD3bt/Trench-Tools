...
function printUsage() {
  console.error(
    "Usage: node scripts/launchdeck-runtime-control.js <start|stop|restart>"
  );
}
...
function argsFor(scriptPath) {
  if (isWindows) {
    return ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", scriptPath, "--mode", "ld"];
  }
  return [scriptPath, "--mode", "ld"];
}
...
(async () => {
  if (action === "restart") {
    await run("stop");
    await run("start");
    await printStartupDiagnostics({ includeExecution: false, includeLaunchdeck: true });
    return;
  }
  await run(action);
  if (action === "start") {
