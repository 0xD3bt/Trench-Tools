(function initLaunchDeckExtensionBootstrap(global) {
  ...
  // Shared migration/constants attached by src/shared/storage-migrations.js.
  // That script is loaded before this one in launchdeck/index.html, and is
  // also imported for side effects by the service worker's index.js, so both
  // surfaces see identical keys and logic.
  ...
  function routeBackendByFeature(pathname, backendState) {
    // Manual trading and execution-owned bridge paths stay on the execution host.
    ...
    // Everything else in the embedded LaunchDeck shell belongs to the
    // LaunchDeck control-plane host.
